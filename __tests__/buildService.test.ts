...
describe('BuildService', () => {
  ...
  describe('findTargetBuild', () => {
    ...
    it('should find build by version and build number', async () => {
      ...
      expect(mockClient.getAppByBundleId).toHaveBeenCalledWith('com.example.app')
    ...
    it('should throw error if no builds found', async () => {
      ...
      await expect(buildService.findTargetBuild())
        .rejects.toThrow('No builds found for version 1.0.0')
    })
    ...
    it('should throw error if specific build not found', async () => {
      ...
      await expect(buildService.findTargetBuild())
        .rejects.toThrow('Build not found with version 1.0.0 and build number 100')
    })
  })
  ...
  describe('waitForProcessing', () => {
    ...
    it('should throw error if build fails', async () => {
      ...
      await expect(buildService.waitForProcessing(mockBuildInfo))
    ...
    it('should timeout after configured time', async () => {
      ...
      await expect(promise).rejects.toThrow('Timeout waiting for build processing after 600 seconds')
